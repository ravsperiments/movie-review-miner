...
from review_aggregator.utils.log_id import generate_log_id, set_log_id
from review_aggregator.utils.logger import get_logger, setup_pipeline_logging

logger = get_logger(__name__)

# Database paths for different modes (stored in data/ folder at project root)
PROJECT_ROOT = Path(__file__).parent.parent
DB_PATHS = {
    "prod": PROJECT_ROOT / "data" / "local.db",
    "test": PROJECT_ROOT / "data" / "test.db",
}
...
def setup_database(mode: str) -> None:
    ...
    from review_aggregator.db.sqlite_client import set_db_path
...
async def crawl(limit: int | None = None, dry_run: bool = False) -> list[str]:
    ...
    from review_aggregator.pipeline.crawl_fetch_links import orchestrate_fetch_links
    from review_aggregator.pipeline.crawl_posts import parse_posts
    from review_aggregator.db.scraper_queries import get_pending_pages_to_parse
    ...
    if pages:
        logger.info("Parsing %d pages...", len(pages))
...
async def extract(
    ...
    from review_aggregator.pipeline.extract_review import run_extract_pipeline
...
async def enrich() -> None:
    ...
    from review_aggregator.pipeline import enrich_movie_data
...
async def run_eval(
    ...
    from review_aggregator.eval.runner import run_evaluation
